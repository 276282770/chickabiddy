...
export default class PanelExchangeBuy2 extends cc.Component {
    ...
    setData(totalEggCount: number, rate: number, type: ExchangeType): void {
        ...
        if (type == ExchangeType.egg2egg) {
            ...
            this.txtTitle.string = "共有" + Math.floor(totalEggCount*rate).toString() + "个待兑换的鸡蛋";
        }else{
            this.txtTitle.string = "共有" + Math.floor(totalEggCount*rate).toString() + "金币可兑换";
        }
    }
    //增加
    onAdd() {
    ...
    //减少
    onReduce() {
        let redNum=this._rate>0?1:1/this._rate;
